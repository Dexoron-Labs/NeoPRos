...
/* Палитра по умолчанию: DEFAULT.THM из x16-PRos (RGB 0-63 на канал).
 * Записывается в DAC VGA (порты 0x3C8/0x3C9), поэтому текстовый режим
 * выглядит так же, как терминал x16-PRos с темой по умолчанию. */
static const uint8_t vga_default_palette[16][3] = {
    ...
    { 3, 14, 17 },  /* 8  тёмн.сер*/
    { 50, 19, 5 },  /* 9  голубой */
    { 33, 53, 22 }, /* 10 светло-зелёный */
    { 16, 53, 56 }, /* 11 светло-циан */
    { 53, 17, 20 }, /* 12 светло-красный */
    { 32, 37, 37 }, /* 13 светло-фиолет */
    { 56, 55, 27 }, /* 14 жёлтый  */
    { 63, 63, 63 }, /* 15 ярко-белый */
};

/* Запись палитры в DAC VGA (16 записей: все цвета текстового режима). */
static void vga_set_palette(void)
{
    outb(0x3C8, 0);             /* начать запись с записи 0 */
    for (int i = 0; i < 16; i++) {
        outb(0x3C9, vga_default_palette[i][0] & 0x3F);
        outb(0x3C9, vga_default_palette[i][1] & 0x3F);
        outb(0x3C9, vga_default_palette[i][2] & 0x3F);
    }
}
...
void vga_init(void)
{
    ...
    vga_set_palette();          /* тема DEFAULT.THM (как x16-PRos) */
    vga_clear();
}
